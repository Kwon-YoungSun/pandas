...
# 서울시내 중학교 진학률 데이터셋
file_path = '../../datasets/part7/2016_middle_shcool_graduates_report.xlsx'
...
# 원핫인코딩(더미 변수)
# 모형이 인식할 수 없는 문자열 데이터를 더미 변수로 변환 ===> day07 ex12.py 참고
from sklearn import preprocessing

label_encoder = preprocessing.LabelEncoder()    # label encoder 생성
onehot_encoder = preprocessing.OneHotEncoder()  # one hot encoder 생성

# label encoder로 문자열 범주를 숫자형 범주(원핫벡터)로 변환
onehot_location = label_encoder.fit_transform(df['지역'])
onehot_code = label_encoder.fit_transform(df['코드'])
onehot_type = label_encoder.fit_transform(df['유형'])
onehot_day = label_encoder.fit_transform(df['주야'])

# 생성된 원핫벡터(열)를 데이터프레임에 새로운 열로 추가 
...
# DBSCAN 모형 객체 생성
# ? 예제대로 하면 5개 값이 아니라 더 많은 값이 나타남
dbm = cluster.DBSCAN(eps=0.2, min_samples=5)    # 밀도 계산의 기준이 되는 반지름 R(eps=0.2)과 최소 포인트 개수 M(min_samples=5)을 옵션에 설정
...
# ---------------------------------------------------------------------------------
# 2. X2 데이터셋에 대하여 위의 과정을 반복(과학고, 외고국제고, 자사고 진학률 + 유형)
# 학교 설립 유형(국립, 공립, 사립)을 추가하여 분석
...
# %%
# ----------------------------------------------------------------------------------------------
# 3. DBSCAN 모형에 입력하는 속성을 2개로 줄여서 예측
# Noise가 현저히 줄어듦
